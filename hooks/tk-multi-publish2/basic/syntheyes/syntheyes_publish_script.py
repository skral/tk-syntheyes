...
class syntheyesSessionPublishPlugin(HookBaseClass):
    ...
    def accept(self, settings, item):
        ...
        if not path:
            ...
            self.logger.warn(
                "The syntheyes script has not been saved.",
                extra=_get_save_as_action(path)
            )

        self.logger.info(
            "syntheyes '%s' plugin accepted the current syntheyes scene." %
            (self.name,)
        )
        return {
            "accepted": True,
...
def _get_save_as_action(path):
    ...
    return {
        "action_button": {
            ...
            "callback": hlev.SetSNIFileName(path)
        }
    }
