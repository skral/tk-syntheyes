...
class SynthEyesActions(HookBaseClass):
    ...
    ADD_SHOT_PRETTY_NAME = "Add Shot"
    CHANGE_SHOT_PRETTY_NAME = "Change Shot Images"
    ...
    def generate_actions(self, sg_publish_data, actions, ui_area):
        ...
        if "change_shot" in actions:
            action_instances.append({
                "name": self.CHANGE_SHOT_PRETTY_NAME,
                "params": None,
                "caption": "Change the images for the current shot.",
                "description": (
                    "This will not add a new shot into the scene and will instead replace the current images"
                )
            })

        if "add_shot" in actions:
            action_instances.append({
                "name": self.ADD_SHOT_PRETTY_NAME,
                "params": None,
                "caption": "Add a new shot into the scene.",
                "description": (
                    "This will add a new shot into the scene."
                )
            })
    ...
    def execute_action(self, name, params, sg_publish_data):
        ...
        # resolve path
        # toolkit uses utf-8 encoded strings internally and SynthEyes API
        ...
        if name == self.ADD_SHOT_PRETTY_NAME:
            self._do_add_shot(path, sg_publish_data)
        elif name == self.CHANGE_SHOT_PRETTY_NAME:
            self._do_change_shot(path, sg_publish_data)

    def _do_change_shot(self, path, sg_publish_data):
        connection = get_existing_connection()

        # if not os.path.exists(path):
        #     # if not Sequence.isSequence(path):
        #     raise TankError("File not found on dist - '%s'" % path)
        # else:
        #     # _sequence = Sequence.sequence_from_pattern(path)
        dir_path = os.path.dirname(path)
        path = os.path.join(dir_path,os.listdir(dir_path)[1])

        image_extensions = SynthEyesActions.IMAGE_EXTENSIONS
        shots = connection.Shots()
        if not shots:
            raise TankError("No Shots found in scene!")

        connection.BeginShotChanges(shots[0])
        shots[0].Set("nm", path)
        connection.AcceptShotChanges(shots[0], "Change Shot Images")

    def _do_add_shot(self, path, sg_publish_data):
        """
        Create a reference with the same settings SynthEyes would use
        ...
        # else:
        #     # _sequence = Sequence.sequence_from_pattern(path)
        #     # path = _sequence[_sequence.first_frame]
        dir_path = os.path.dirname(path)
        path = os.path.join(dir_path,os.listdir(dir_path)[1])

        connection.AddShot(path, asp=0.0)
